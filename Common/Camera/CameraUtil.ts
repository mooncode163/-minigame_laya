...
export default class CameraUtil {
  ...
  // }
  WorldToCanvasPoint(camera, pt) { 
    var size = CameraUtil.main.GetWorldSize(camera);
    var sizeCanvas = Common.sizeCanvas; 
    var x = (pt.x-(-size.x / 2))*sizeCanvas.width/size.x;
    var y =(size.y / 2-pt.y)*sizeCanvas.height/size.y;
    return new Laya.Vector3(x, y,0);
  }

  CanvasToWorldPoint(camera, pt) {
    // 坐标原点在中心
    var x_canvas = pt.x;
    var y_canvas = pt.y;
    var size = CameraUtil.main.GetWorldSize(camera);
  ...
  WorldToScreenPoint(camera, pt) {
    var size = CameraUtil.main.GetWorldSize(camera);
    var sizeCanvas = Device.main.screenSize; 
    var x = (pt.x-(-size.x / 2))*sizeCanvas.width/size.x;
    var y = (size.y / 2-pt.y)*sizeCanvas.height/size.y;
    return new Laya.Vector3(x, y,0);
  }

  ScreenToWorldPoint(camera, pt) {
    // 坐标原点在中心
    var x_canvas = pt.x;
    var y_canvas = pt.y;
    var size = CameraUtil.main.GetWorldSize(camera);
  ...
  CanvasToWorldWidth(camera, w) {
    // 坐标原点在中心 
    var size = CameraUtil.main.GetWorldSize(camera);
    var sizeCanvas = Common.sizeCanvas;
    return w * size.x / sizeCanvas.width;  
  }

  CanvasToWorldHeight(camera, h) {
    // 坐标原点在中心 
    var size = CameraUtil.main.GetWorldSize(camera);
    var sizeCanvas = Common.sizeCanvas;
    return h * size.y / sizeCanvas.height;  
  }
