...
/*
* 滚动视图  https://blog.csdn.net/honey199396/article/details/79716679
*/
export default class ScrollView extends Laya.Box implements Laya.IRender, Laya.IItem {

    public direction = ScrollViewDirection.Vertical;
    ...
    // 上边距
    private _upAlign: number = 0;
    //下边距
    private _downAlign: number = 0;
    // 单元格集合
    ...
    public set upAlign(v: number) {
        this._upAlign = v;
    }
    public get upAlign(): number {
        return this._upAlign;
    }


    public set downAlign(v: number) {
        this._downAlign = v;
    }
    public get downAlign(): number {
        return this._downAlign;
    }
    ...
    private refreshCellsPos() {
        ...
        for (var i = 0; i < cellCount; i++) {
            ...
            {
                let posY: number = this.getCellPosByIndex(i);
                cell.pos(this.width/2, posY);
            }
        ...
        if(this.direction==ScrollViewDirection.Vertical)
        {
            this.height = this._upAlign + cellCount * this._cellHeight + (cellCount - 1) * this.space + this._downAlign;
        }
    }
    ...
    public getCellPosByIndex(index: number): number {
        ...
        if(this.direction==ScrollViewDirection.Vertical)
        {
            return this._upAlign + (index + 0.5) * this._cellHeight + index * this.space;
        }
